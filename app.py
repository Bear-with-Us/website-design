...
import os
# Make sure logging is set up here if it isn't globally very early
# app.logger.info(...) can be used if app object is already created and configured with logger

db_file_path = '/home/deployer/app/essentials/instance/VR3.db'
db_dir_path = '/home/deployer/app/essentials/instance'

print(f"--- PATH DIAGNOSTICS ---") # For direct Gunicorn output or journalctl
print(f"Checking DB file: {db_file_path}")
print(f"DB file exists? {os.path.exists(db_file_path)}")
print(f"DB file is file? {os.path.isfile(db_file_path)}")
if os.path.exists(db_file_path):
    print(f"DB file readable? {os.access(db_file_path, os.R_OK)}")
    print(f"DB file writable? {os.access(db_file_path, os.W_OK)}") # Less crucial for read, but informative
    try:
        file_size = os.path.getsize(db_file_path)
        print(f"DB file size: {file_size} bytes")
    except OSError as e:
        print(f"Could not get DB file size: {e}")


print(f"Checking DB directory: {db_dir_path}")
print(f"DB dir exists? {os.path.exists(db_dir_path)}")
print(f"DB dir is directory? {os.path.isdir(db_dir_path)}")
if os.path.exists(db_dir_path):
    print(f"DB dir readable (execute bit)? {os.access(db_dir_path, os.X_OK)}") # Need X to list/access files in it
    print(f"DB dir writable? {os.access(db_dir_path, os.W_OK)}") # Crucial for SQLite commits
print(f"--- END PATH DIAGNOSTICS ---")
...
def get_block():
    ...
    elif group_val == 'Normal' and date_val == 0:
        return jsonify({"html": render_template("Day1-normal.html", user=user)})
...
def add_player():
    ...
    VIP_starting_time = datetime(2025, 5, 16, 20, 0, 0, tzinfo=ZoneInfo("Asia/Shanghai"))
    Normal_starting_time = datetime(2025, 5, 17, 12, 0, 0, tzinfo=ZoneInfo("Asia/Shanghai"))
    if not user_id:
